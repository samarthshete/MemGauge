...
## 4. Phase 2 — exact ordered sequence (awaiting approval; incurs cost)

1. `fly auth whoami` (install `flyctl` + `fly auth login` first if needed — needs user).
2. Provision managed services (**needs user signups/credentials — hard stop**):
   Postgres w/ pgvector (Neon or Supabase; confirm `CREATE EXTENSION vector`),
   Neo4j Aura Free, Upstash Redis (TLS `rediss://`).
3. One-time schema: `psql "$DIRECT_DSN" -v ON_ERROR_STOP=1 -f app/db/init.sql`; verify with `\dt`.
4. `fly launch --no-deploy` (registers the app from the committed `fly.toml`).
5. `fly secrets set` — names below; `MEMGAUGE_API_TOKEN=$(openssl rand -hex 32)` revealed to the owner once, never committed.
6. `fly deploy`.
7. Live verification against the real URL: `GET /healthz` all `up`; `GET /metrics`
   serves `memgauge_*`; unauthenticated `POST /v1/memories` → **401**;
   `POST /v1/eval/run` with bearer → run persists (also creates Neo4j
   constraints); `GET /report/{run_id}` renders.
8. Update STATUS.md (Phase 11 deploy → VERIFIED + URL + date), DEPLOY.md
...
`NEO4J_USER`, `NEO4J_PASSWORD`, `REDIS_URL` (`rediss://`),
`EMBEDDING_MODEL` (recommend `__offline-deploy-model__` → deterministic hash
embedder, no model download), `RATE_LIMIT_PER_MIN` (60), `CORS_ALLOW_ORIGINS`
(empty = no browser origins).

**Estimated setup + cost:** four provider accounts (Fly.io — requires a credit
card — plus Neon/Supabase, Neo4j Aura, Upstash). Data services all on free
tiers → $0. Fly `shared-cpu-1x`/512 MB with auto-stop and `min_machines_running=0`
→ roughly **$0–5/month** (≈$3/mo if pinned always-on). Setup effort ≈ 1–2 hours.
